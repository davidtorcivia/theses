...
The first visit shows `/setup`: it creates the owner account, enrols an authenticator and signs you in. Until that is done every other route redirects there. Invitation and password reset links are written to the log at info level; mail is wired in a later step.
