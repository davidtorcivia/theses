...
| `THESES_DATA_DIR` | Where `theses.db` lives. |
...
mounted at the data directory and holds the database.

## Behind a reverse proxy

Put any reverse proxy in front of the published port. One site block does it:
match the public host name, terminate TLS, forward to `127.0.0.1:8080`, and
pass the client address through as `X-Forwarded-For`. Nothing else about the
request has to be rewritten.
...
Two places hold everything an install owns: the data directory, which holds
the SQLite database, and the object storage bucket. Copy both. A nightly
archive from the data directory to the bucket under a write-only key is
planned; until it ships, back the volume up from the host.
