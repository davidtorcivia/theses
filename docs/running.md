...
`THESES_LOG_LEVEL`, `THESES_BIND_HOST`, `THESES_PORT`, `TZ` and
...
nothing off the host reaches it directly. One volume, `theses-data`, holds the
database, the markdown mirror and the thumbnail cache.

## Behind a reverse proxy

Put any reverse proxy in front of the published port and give it one site
block. As a Caddyfile:
...
named: the database now, the object store and the age of the last backup
later.

## Backups

Two places hold everything an install owns: the data directory, which is the
SQLite database, the markdown mirror and the thumbnail cache, and the object
storage bucket. Copy both. A nightly archive from the data directory to the
bucket under a write-only key is planned; until it ships, back the volume up
from the host.
